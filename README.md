# 領収書自動発行ツール
## 使用方法
右上の緑の ＜code＞ ボタンを押すと、 Download ZIP が出てくるので、 ZIP 形式で自身のPCにインストールしてください。
解凍後、ダウンロードしたディレクトリで以下のマニュアルに従って発行してください。

1枚のみ(上段と下段で同じ内容を発行) の場合と、 2枚(上段と下段で異なる内容を発行) の場合で操作内容が異なりますので注意してください。

よくあるエラー内容を下にまとめました。参考にしてください。
